...
def plot_efficient_frontier(results_df, max_sharpe, min_vol):
    ...
    st.pyplot(fig)
def apply_lstm_correction(train, forecast, test):
    
    train_pred = forecast[:len(train)]['yhat'].values
    ...
    for i in range(len(res_scaled)-lookback):
        X.append(res_scaled[i:i+lookback])
        y.append(res_scaled[i+lookback])
    ...
    last_seq = res_scaled[-lookback:]
    pred_res = []

    for _ in range(len(test)):
        pred = model.predict(last_seq.reshape(1,lookback,1), verbose=0)
        pred_res.append(pred[0][0])
        last_seq = np.append(last_seq[1:], pred).reshape(-1,1)

    pred_res = scaler.inverse_transform(np.array(pred_res).reshape(-1,1)).flatten()

    return pred_res, residuals
    # Calculate residuals
    df_prophet['residual'] = df_prophet['y'] - df_prophet['yhat']

    scaler = MinMaxScaler()
    scaled = scaler.fit_transform(df_prophet[['residual']])
    ...
    # Predict full residual sequence
    predicted_residuals = model.predict(X)

    predicted_residuals = scaler.inverse_transform(predicted_residuals)

    # Align sizes
    hybrid = df_prophet['yhat'].values[lookback:] + predicted_residuals.flatten()

    return hybrid, df_prophet['ds'].values[lookback:]
def add_atr_regime(df):
    df['TR'] = np.maximum(
        df['High'] - df['Low'],
        np.maximum(
    ...
    df['regime'] = df['ATR'].apply(
        lambda x: 'HIGH' if x > (atr_mean + atr_std) else 'LOW'
    )
...
def main():
    ...
    def main():
     ...
     st.header("2. AdaProphet Forecast")

    df_prophet = hist.reset_index()[['Date','Close','regime']]
    df_prophet.columns = ['ds','y','regime']
...
train = df_prophet[:split].copy()
test = df_prophet[split:].copy()

# Adaptive Prophet
if train['regime'].iloc[-1] == 'HIGH':
    m = Prophet(growth='logistic', seasonality_mode='multiplicative')
    train.loc[:, 'cap'] = train['y'].max() * 1.2
    m.fit(train[['ds','y','cap']])
    cap_value = train['cap'].max()
else:
...
if cap_value is not None:
    ...
    df_prophet['yhat'] = forecast['yhat'][:len(df_prophet)]
    hybrid_prediction, hybrid_dates = apply_lstm_correction(df_prophet)
    st.header("Hybrid Prophet + LSTM Forecast")
    ...
    fig_hybrid.update_layout(title="Final Hybrid Forecast")
